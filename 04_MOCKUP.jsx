...
const TreasureHuntMockup = () => {
  ...
  const [connectedAddress] = useState('0xABCD1234567890abcdef1234567890abcdef1234'); // Mock address
  const [walletDropdownOpen, setWalletDropdownOpen] = useState(false);

  // Network State (mock - can toggle via ?wrongNetwork=1 query param)
  const [isWrongNetwork, setIsWrongNetwork] = useState(() => {
    if (typeof window !== 'undefined') {
      return new URLSearchParams(window.location.search).get('wrongNetwork') === '1';
    }
    return false;
  });
  ...
  // Check for missed discoveries on mount
  useEffect(() => {
  ...
  const showFullDiscovery = (amount) => {
    ...
    const discovererShare = amount * 0.50;
    const stakerShare = stakedHunt > 0 && hasQualifyingBet ? amount * 0.40 : 0;
    setBalance(prev => prev + discovererShare);

    setTimeout(() => {
      // Flavor + outcome for discoverer
      addLog("The discoverer's share fills the coffers.", 'reward');
      addLog(`Received: ${discovererShare.toFixed(2)} USDC (Discoverer's share)`, 'result');
    }, 800);
    setTimeout(() => {
      if (stakerShare > 0) {
        setPendingRewards(prev => prev + stakerShare);
        // Flavor + outcome for stakers
        addLog("The loyal crew divides their portion.", 'reward');
        addLog(`Pending: ${stakerShare.toFixed(2)} USDC (Crew share)`, 'result');
      }
    }, 1600);
    setTimeout(() => addLog("The map shudders with newfound power.", 'map'), 2400);
    setTimeout(() => {
      const overflow = J + (amount * 0.5) - amount;
      setJ(Math.max(0, overflow));
      setM(prev => prev * 2);
      setEpochId(prev => prev + 1);
      setHasQualifyingBet(false);
      setLastSeenEpochId(prev => prev + 1);
      addLog("Discovery: Expedition advanced — Chest reset", 'expedition');
    }, 3200);
    setTimeout(() => {
      setDiscoveryAnimation(null);
      setMapGlow(false);
  ...
  const disconnectWallet = () => {
    // In real app, would call wagmi disconnect
    setWalletDropdownOpen(false);
    // For mock, just close dropdown
  };

  const triggerDemoDiscovery = () => showFullDiscovery(M);

  const progressPercent = (J / M) * 100;
  const currentMapPrice = getMapPrice(mapSupply);
      ...
      <div className="max-w-5xl mx-auto relative z-10">
            ...
            >
              {isWrongNetwork ? (
                ...
                <>
                  ...
                  <span className="font-fell text-sm" style={{ color: '#93c5fd' }}>Base</span>
                </>
              )}
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <ParchmentPanel className="p-4">
            ...
            <button onClick={triggerDemoDiscovery}
          ...
          <ParchmentPanel className="p-4">
            ...
            <div className="space-y-2">
              ...
              {pendingRewards > 0 && (
                <WoodButton onClick={claimRewards} className="w-full py-2">
                  Claim Yer Rewards
                </WoodButton>
              )}
            </div>
          ...
          <ParchmentPanel className="p-4">
            ...
            {huntBalance > 0 && !cooldownStart && (
              <div className="mb-3">
               ...
               />
                <WoodButton onClick={() => stake(stakeAmount)} className="w-full mt-2 py-2">
            ...
            {stakedHunt > 0 && !cooldownStart && (
              <WoodButton onClick={initiateWithdraw} variant="secondary" className="w-full py-2">
                Prepare to Disembark
              </WoodButton>
            )}

            {cooldownStart && cooldownRemaining > 0 && (
              <WoodButton onClick={cancelWithdraw} variant="secondary" className="w-full py-2">
                Cancel Disembarkation
              </WoodButton>
            )}

            {cooldownStart && cooldownRemaining === 0 && (
              <WoodButton onClick={completeWithdraw} className="w-full py-2">
                Complete Withdrawal
              </WoodButton>
            )}
          </ParchmentPanel>
                  ...
                  {mapTradeMode === 'buy' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={mapPurchasing || !mapPurchaseAmount || parseFloat(mapPurchaseAmount) <= 0}
                        className="w-full py-3 rounded font-fell transition-opacity"
                        style={{
                          background: mapPurchasing ? '#8b7355' : '#5c4a32',
                          color: '#f5ece0',
                          opacity: (!mapPurchaseAmount || parseFloat(mapPurchaseAmount) <= 0) ? 0.5 : 1,
                          cursor: (mapPurchasing || !mapPurchaseAmount || parseFloat(mapPurchaseAmount) <= 0) ? 'not-allowed' : 'pointer',
                        }}
                      >
                  ...
                  {mapTradeMode === 'sell' && (
                    <>
                      ...
                      <button
                        ...
                        disabled={mapSelling || !mapSellAmount || parseFloat(mapSellAmount) <= 0 || parseFloat(mapSellAmount) > mapBalance}
                        className="w-full py-3 rounded font-fell transition-opacity"
                        style={{
                          background: mapSelling ? '#8b5555' : '#8b4040',
                          color: '#f5ece0',
                          opacity: (!mapSellAmount || parseFloat(mapSellAmount) <= 0 || parseFloat(mapSellAmount) > mapBalance) ? 0.5 : 1,
                          cursor: (mapSelling || !mapSellAmount || parseFloat(mapSellAmount) <= 0 || parseFloat(mapSellAmount) > mapBalance) ? 'not-allowed' : 'pointer',
                        }}
                      >
